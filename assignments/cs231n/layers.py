...
def affine_backward(dout, cache):
    ...
    for i in range(dx_t.shape[0]):
      dx[i] = dx_t[i].reshape(x.shape[1:])
...
def svm_loss(x, y):
    ...
    loss = 0
    dx = np.zeros(x.shape)
    for t_index, t in enumerate(x):
      y_times = 0
      for x_index, x_i in enumerate(t):
        if(x_index == y[t_index]):
          continue
        
        temp = x_i - t[y[t_index]] + 1
        if(temp > 0):
          loss += temp
          dx[t_index][x_index] = 1
          y_times += 1

      dx[t_index][y[t_index]] = -y_times
...
def softmax_loss(x, y):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    loss = 0    
    dx = np.diag(1/np.sum(np.exp(x), axis=1)) @ np.exp(x)

    for t_index, t in enumerate(x):
      loss += np.log(np.sum(np.exp(t))) - t[y[t_index]]
      dx[t_index][y[t_index]] -= 1
